...
// Each folder represents a route segment that maps to a URL segment. To create 
// a nested route, you can nest folders inside each other.
...
// accessible because it does not have a corresponding page.js file. This folder 
// could be used to store components, stylesheets, images, or other colocated files.

// Creating UI
// Special file conventions are used to create UI for each route segment. The most 
// common are pages to show UI unique to a route, and layouts to show UI that is 
...
// Tree: A convention for visualizing a hierarchical structure. For example, 
// a component tree with parent and children components, a folder structure, etc.
...
//  other routes in the pages directory for previous behavior. If your application 
// uses the pages directory, please also see the Pages Router documentation.

// Good to know: The App Router takes priority over the Pages Router. Routes across 
// directories should not resolve to the same URL path and will cause a build-time 
// error to prevent a conflict.

// Next.js App Directory
// By default, components inside app are React Server Components. This is 
// a performance optimization and allows you to easily adopt them, and you can 
...
// following the file-system hierarchy from the root folder down to a final leaf 
// folder that includes a page.js file. See Defining Routes.
// Files are used to create UI that is shown for a route segment. See special files.

// Route Segments
// Each folder in a route represents a route segment. Each route segment is mapped 
...
// you can add a new /dashboard/settings route by nesting two new folders in the 
// app directory.
...
//  page.js or route.js are publicly addressable.
...
// Parallel Routes: Allow you to simultaneously show two or more pages in the same 
// view that can be navigated independently. You can use them for split views that 
// have their own sub-navigation. E.g. Dashboards.

// Intercepting Routes: Allow you to intercept a route and show it in the context 
// of another route. You can use these when keeping the context for the current 
// page is important. E.g. Seeing all tasks while editing one task or expanding 
// a photo in a feed.
