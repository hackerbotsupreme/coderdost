...
// next/font/local. For features and usage, please see the Optimizing Fonts page.
...
// src
// The path of the font file as a string or an array of objects 
// (with type Array<{path: string, weight?: string, style?: string}>) 
...
// A string with possible values of the weights available for 
// the specific font or a range of values if it's a variable font
...
// the possible values are '100', '200', '300', '400', '500', '600', '700', 
// '800', '900' or 'variable' where 'variable' is the default)
...
// An array of style values if the font is not a variable google font. It applies 
// to next/font/google only.
// Used in next/font/google and next/font/local
...
// style: 'oblique': A string - it can take any value for next/font/local but 
// is expected to come from standard font styles
// style: ['italic','normal']: An array of 2 values for next/font/google 
// - the values are from normal and italic
...
// the font weight is included to keep the file size down. The possible values 
// of axes depend on the specific font.
...
// axes: ['slnt']: An array with value slnt for the Inter variable font which has 
// slnt as additional axes as shown here. You can find the possible axes values 
// for your font by using the filter on the Google variable fonts page and looking 
// for axes other than wght

// display
// The font display with possible string values of 'auto', 'block', 'swap', 
...
// fallback: ['system-ui', 'arial']: An array setting the fallback fonts to 
// system-ui or arial

// adjustFontFallback
// For next/font/google: A boolean value that sets whether an automatic fallback 
// font should be used to reduce Cumulative Layout Shift. The default is true.
// For next/font/local: A string or boolean false value that sets whether 
// an automatic fallback font should be used to reduce Cumulative Layout Shift. 
...
// is applied with the CSS variable method.
...
// style
// Returns a read-only CSS style object for the loaded font to be passed to 
// an HTML element, including style.fontFamily to access the font family name 
// and fallback fonts.
...
// variable is defined and set the variable option of the font loader object as follows:
...
// hosted as one instance in your application. Therefore, if you need to use 
// the same font in multiple places, you should load it in one place and import 
// the related font object where you need it. This is done using a font definitions file.
